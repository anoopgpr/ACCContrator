...
function getProductList(str)
{
	...
	if(showfilter == 'LEAK BLOCK')
	{
		$("#productliqprice_div").css('display', 'block');
		$("#productbagprice_div").css('display', 'none');
	}
	else
	{
		$("#productliqprice_div").css('display', 'none');
		$("#productbagprice_div").css('display', 'block');
	}
  ...
  function viewtransactionHistory()
	{
		$.ajax({
			url: SERVER2+'get_contractor_transaction_details_json_api.asp',
			type: 'GET',
			timeout: 50000,
			dataType: 'json',
			data: {'apiuid': 'MLOYALAPIACC','apipswd':'Ml0yalAP!2o14ACC','mobileno': user.mobile },
		   ...
		   //called when complete
				...
				else
				{
					 for (var i = 0; i < data.data.length; i++)
					 {
							...
							str1 +='<div class="approval_block"><img src="assets/images/retailer_name_icon.png" class="location_icon"/><span class="customer_name">Name:'+checkforundefined(data.data[i].Member_Name)+'</span></div>';
							str1 +='<div class="approval_block"><img src="assets/images/retailer_call_icon.png" class="location_icon"/><span class="approval_email">Mobile:<a href="tel:'+checkforundefined(data.data[i].Member_Mobile)+'" class="a_link_call_new"  style="font-weight: 500;">'+data.data[i].Member_Mobile+'</a></span></div>';
							str1 +='<div class="approval_block"><img src="assets/images/retailer_mail_icon.png" class="location_icon"/><span class="approval_email">Bill No.: '+checkforundefined(data.data[i].bill_number)+'</span></div>';
							str1 += '</div></div>';
							str1 +='</div>';
					 }
					 ...
					 document.getElementById("my_contractor_history_div").innerHTML=str1;
	
					 $.mobile.changePage( "#TransactionHistory", { transition: "none"} );
...
$(document).on('pageshow', '#addtransactions', function (event, ui) {
	...
	//get_item_master();
		...
		document.getElementById("tx_date").setAttribute("min", "2023-01-01");
...
function tx_Submit()
{
	...
	var dealercode = $('#tx_dealercode').val();
	...
	if(ihbmobile.match(numbers)){
		var ihbmobile1 = ihbmobile;
	}else{
		var ihbmobile1 = '';
	}
	...
	}else if(quantity1.trim() == '' || quantity1 == '0'){
		toast('Please check bag quantity');
	}else if(contractType == '' || contractType == null || contractType == 'undefined' || contractType == 'null'){
		toast('Please Select Contractor Type');
	}else if(ihbmobile1 == '' || ihbmobile1 == null || ihbmobile1 == 'undefined' || ihbmobile1 == 'null'){
		toast('Please check IHB Mobile no.');
	}else if(ihbname.trim() == ''){
		toast('Please enter IHB name');
	}else if(dealercode == '' || dealercode == null || dealercode == 'undefined' || dealercode == 'null'){
		toast('Please select dealer');
	}else{
		checkfor_siteValidation();
	}
}

function checkfor_siteValidation()
{
	document.getElementById("create_new_transaction").disabled = true;
	var ihbmobile = $('#tx_ihbmobile').val();
	var ihbname = $('#tx_ihbname').val();
	 ...
	 $.ajax({
			   ...
			   success: function(data, textStatus, xhr) {	
					   ...
					   if(data.sms.length>=1)
					   {
						   ...
						   if(data.sms[0].ContractorNO==user.mobile)
						   {
								tx_Submit_2();
						   }
						   else
					   ...
					   {
							add_new_site(ihbmobile,ihbname,ihbaddress,contractType);
					   }
			   },
			   error: function(xhr, textStatus, errorThrown) {
...
function add_new_site(ihbmobile,ihbname,ihbaddress,contractType)
{
...
{
	console.log('enter in tx_submit1')

	var contractType = $("#tx_matrialstatus").val();
	var product = $('input[name="tx_product"]:checked').val();
	var protype = product.split('|')
	console.log(protype[1]);
	if(protype[1] == 'Bag'){
	var quantity = $('#tx_quantity').val();
	}else{
	var quantity = $('#tx_quantityLiquid').val();
	...
	var showfilter_new = $('#tx_showproductfilter').val();
	console.log(showfilter_new);
    var Bprice = '';
	if(showfilter_new=='Cement' || showfilter_new=='DRY MIX')
	{
		Bprice = $('#tx_quantitybagprice').val();
	}
	else if(showfilter_new=='LEAK BLOCK')
	{
		Bprice = $('#tx_quantityLiquidprice').val();
	}
	...
	else if(quantity.trim()=='' || quantity=='0')
	{
		console.log('1111')
		toast('Please check bag quantity');
	}
	...
	// }
	else if(ihbmobile=='' || ihbmobile==null || ihbmobile=='undefined' || ihbmobile=='null')
	{
		toast('Please check IHB Mobile no.');
	}
	else if(ihbname.trim()=='')
	...
	else if(Bprice=='' || Bprice==null || Bprice=='undefined' || Bprice=='null')
	{
		toast('Please select enter Price');
	}
	else
	{
		...
		$.ajax({
			...
			success: function(data, textStatus, xhr) {
				...
				else
				{
					...
					document.getElementById("create_new_transaction").disabled = false;
					document.getElementById("productlist_div").innerHTML= '';
					$('#tx_pincode').val('');
					$('#tx_ihbname').val('');
					$('#tx_address').val('');
					$('#tx_quantity').val('');
					$('#tx_ihbmobile').val('');
					$('#tx_quantityLiquid').val('');
					$('#tx_dealercode_new').val('');
					$('#tx_quantitybagprice').val('');
					$('#tx_quantityLiquidprice').val('');
					$.mobile.changePage( "#homepage", { transition: "none"} );

					update_sites_contractor(ihbmobile);
...
function update_sites_contractor(ihbmobile)
{
	console.log('enter in get ihb curstomer detAILS')
	console.log(ihbmobile + ' ' + user.mobile + 'do not enter in this function');
	$.ajax({
				url: SERVER2+'get_ihb_customer_details.asp?apiuid=MLOYALAPI&apipswd=Ml0yalAP!2o14',
				type: 'GET',
				timeout: 50000,
				dataType: 'json',
				data: { 'mobileno': ihbmobile,'contractorno': user.mobile},
			   success: function(data, textStatus, xhr) {	
...
{
	// console.log('enter funtion tx_submit_2 for transaction ')
	// var contractType = $("#tx_matrialstatus").val();
...
function showDealerListbyName(){
	document.getElementById("searchlist_div").innerHTML = '';
	$('#tx_smobile').val('');
	var districtid = $('#tx_district').val();
	var dealerId = $('#tx_dealercode_new').val();
	if(districtid){
	$.ajax({
	  url:SERVER2+'get_dealer_by_dealer_name_and_district_json_api.asp?apiuid=MLOYALAPIACC&apipswd=Ml0yalAP!2o14ACC&Store_Name='+ dealerId +'&Store_District='+districtid,
	  type: 'GET',
	  timeout: 50000,
	  dataType: 'json',
	 ...
	 console.log(data);
	       var stringData = JSON.stringify(data);
	        localStorage.setItem('searchDealer',stringData);
		  var listItems= "";
			  listItems+= "<option value=''  disabled='disabled' style='background-color:#3E3E3E; font-size: 15px; '  selected='selected'>Select Dealer</option>";
		  for (var i = 0; i < data.data.length; i++)
		  {       
		...
		toast('Please Select District First.');
	}
}
...
function SelectedDealerName(str){
	...
	// console.log(dom2.data);
	for (var l = 0; l < dom2.data.length; l++){
		// console.log(dom2.data[l].Store_Code);
		if(str == dom2.data[l].Store_Code){
			// console.log(dom2.data[l].Store_Code);
		    $('#tx_dealercode').val(dom2.data[l].Store_Code);
			$('#tx_dealercode_new').val(dom2.data[l].Store_Name);
			document.getElementById("searchlist_div").innerHTML = '';
...
function forget_pswd_confirm(){
	     document.getElementById("Login_Otp_Send").disabled = true;
		 ...
		 else
		 {
			...
			document.getElementById("Login_Otp_Send").disabled = false;	 
		 }
}
	 ...
	 else if(mob.length==10)
	 {
		 ...
		 $.ajax({
			...
			success: function(data, textStatus, xhr) {
				 ...
				 if(data.data == '')
				 {
					toast('This number not registered');
					$("#wait").css("display","none");
					document.getElementById("Login_Otp_Send").disabled = false;
				 }
				 else
				 {
						var conName = data.data[0].CustomerName
						if(conName != ''){
						$.ajax({
							url: SERVER+'forgot_pswd.asp',
							type: 'GET',
...
function authenticate_new()
{
 ...
 if(uname=='' || pwd==''){
  ...
  $.ajax({
   ...
   timeout: 50000,
    ...
    success: function(data, textStatus, xhr) {
	console.log(data);
	// console.log(data.Area_Office_code);
	// console.log("success"+JSON.stringify(data));
   if(data.status=='success'){
	localStorage.setItem('stateID',user.store_state_id);
	user.csostateId = checkforundefined(data.store_state_id);
    user.CSOcode = data.CSO_Code;
	user.balance = data.balance;
	user.areacode = data.Area_Office_code;
	localStorage.setItem('AreaOfficecode',user.areacode);
	...
	user.store_code_atootbandhan = checkforundefined(data.storecode);
	...
	user.sucode = checkforundefined(data.SU_CSI_Code);
	...
	document.getElementById("dealermapName").innerHTML= data.store;
	document.getElementById("dealermapNumber").innerHTML= data.storecode;
	document.getElementById("csomapName").innerHTML= data.CSO_name;
	document.getElementById("csomapNumber").innerHTML= data.CSO_Mobile;
    $('#username').val('');
    $('#password_myacc').val('');
 ...
 if(user.store_code_atootbandhan=='undefined' || user.store_code_atootbandhan=='null' || user.store_code_atootbandhan==null)
	...
	$('#update_firstname').val(user.city);
	$('#update_lastname').val(user.district);
	...
	$('#update_firstname_self').val(user.firstname);
	$('#update_lastname_self').val(user.lastname);
	$('#update_email_self').val(user.emailid);
	$('#update_address').val(user.floor_flat);
	$('#update_mobile').val(user.mobile);
	
	//$('#update_mobile_self').val(user.mobile);

	//$('#update_city').val(user.city);
	//$('#update_district').val(user.district);
	...
	var numbers = /^[0-9]+$/;
	if(checkforundefined(parseInt(user.state))!='')
	{
	...
	if(user.dob!='')
	{
		...
		$('#update_dob_self').val(yyyy+'-'+mm+'-'+dd);
	...
	if(user.firstname=='undefined' || user.firstname=='null' || user.firstname==null || user.firstname=='')
	{
		console.log('111:'+user.source);
		
		/*if(user.source == 'self')
		{
			if(checkforundefined(user.point_claim)!='')
			{
				$('#updateProfile_11').css('display','block');
				$('#updateProfile_22').css('display','none');
				$('#select_ptype_div2').css('display','none');
			}
			else
			{
				$('#updateProfile_11').css('display','none');
				$('#updateProfile_22').css('display','block');
				$('#select_ptype_div2').css('display','block');
			}
	...
	else
	    {
			...
			if(checkforundefined(user.point_claim)!='')
			{
				$('#updateProfile_1').css('display','block');
				$('#updateProfile_2').css('display','none');
				$('#select_ptype_div').css('display','none');
			}
			else
			{
				$('#updateProfile_1').css('display','none');
				$('#updateProfile_2').css('display','block');
				$('#select_ptype_div').css('display','block');
			}

			$.mobile.changePage( "#profilepage", { transition: "flip"} );
		}
		*/
		showPoP("Atoot Bhandhan loyalty progamme is only for registered contractor!.Please get in touch with ACC Sales team if you want to be part of this");
    }
    else
    {  
		...
		{
			showPoP("Your Registration is pending for approval with ACC Sales team.");
		}
...
function authenticate(){
 ...
 if(uname=='' || pwd==''){
  ...
  $.ajax({
   ...
   if(data.status=='success'){
	...
	user.mobile_alt = checkforundefined(data.altmobileno);
	user.gender = checkforundefined(data.Gender);
    user.pincode = checkforundefined(data.PostalCode);
	user.address = checkforundefined(data.Address);
 ...
 if(user.store_code_atootbandhan=='undefined' || user.store_code_atootbandhan=='null' || user.store_code_atootbandhan==null)
	...
	$('#update_email').val(user.emailid);
	$('#update_mobile2').val(user.mobile_alt);
	//$('#update_state').val(user.state);
	$('#update_pincode').val(user.pincode);
	$('#update_aadharno').val(user.aadhar_no);
	$('#update_address').val(user.address);
 ...
 if(uname=='' || pwd==''){
  ...
  $.ajax({
   ...
   if(data.status=='success'){
    ...
    user.dob = data.dob;
    user.emailid = data.emailid;
	user.store_code_atootbandhan = data.storecode;
	user.def_store = data.store;
	user.def_city = data.store_city;
	user.visit_frequency = data.visit_frequency;
	user.membership_slab = data.slabid;
...
function updateProfile(){
	...
	if(firstname.trim() == '' || lastname.trim() == '' || email.trim() == '' || birthday == '')
	{
		...
		// $.mobile.changePage( "#profilepage_from", {transition: "flip"});
	}
	else
	{
		if(checkforundefined(user.point_claim)!='')
...
function updateProfile_1()
{
		...
		//var from_type =$('input[name="profilepage_from_radio"]:checked').val();
...
function updateProfile_11()
{

		var firstname = $('#update_firstname_self').val();
		var lastname = $('#update_lastname_self').val();
		var mobile = user.mobile;
		var email = $('#update_email_self').val();
		var address = '';
		var manpower = '';
		var infrastructure = '';
		var expertise = '';
		...
		var birthday = $('#update_dob_self').val();
...
function updateProfile_22()
{
	...
	{
		$.mobile.changePage( "#profilepage_from_self", {transition: "flip"} );
	}
}

function updateProfile_final()
{
		...
		var email = $('#update_email').val();
		var address = $('#update_address').val();
		var mobile2 = $('#update_mobile2').val();
		var aadharno = $('#update_aadharno').val();
		var state = $('#update_state').val();
		...
		var from_type = $('#select_ptype1').val();
		...
		//var from_type =$('input[name="profilepage_from_radio"]:checked').val();
		var from_type = $('#select_ptype').val();
	...
	else if(typeof from_type==undefined || from_type=='undefined'|| from_type==null|| from_type=='null' || from_type=='')
	{
		toast('Please check claim type.');
	}
	 ...
	 $.ajax({
			...
			timeout: 30000,
		  	data: {
				...
				'doamonth': '',
				'doayear': '',
				'storecd': user.store_code_atootbandhan,
				'source': user.source,
...
function updateProfile_final_2()
{
	...
	var mobile2 = $('#update_mobile2').val();
	var aadharno = $('#update_aadharno').val();
	 var state = user.store_state_id;//$('#update_state').val();
	var district = user.Distict;//$('#update_district').val();
	var city = user.city;//$('#update_city').val();
...
function updateProfile_final_self()
{
		var firstname = $('#update_firstname_self').val();
		var lastname = $('#update_lastname_self').val();
		var mobile = user.mobile;
		...
		var store_code = $('#update_dealer_self').val();
		    store_code = '9110031199';
		...
		var anivesary = $('#update_doa_self').val();
		if(anivesary!=''){
			var dob = anivesary.split('-');
			// console.log(dob);
			dd1 = dob[2];
			mm1 = dob[1];
			yyyy1 = dob[0];		
		}
		//var from_type =$('input[name="profilepage_from_radio_self"]:checked').val();
		var from_type = $('#select_ptype2').val();
...
// function showCityList_update()
// {

//   var stateid = $('#register_state_update').val();

//   $.ajax({
//     url: SERVER +'get_cities_data_json_api.asp',
//     type: 'GET',
//     timeout: 50000,
//     dataType: 'json',
//     data: {
...
// 				listItems+= "<option value='" + data.data[i].city_id + "'>" + data.data[i].city_name + "</option>";
// 		}
// 				  $("#register_city_update").html(listItems);
...
function updateProfile_final_self_2(){
		...
		// 	yyyy1 = dob[0];		
		// }else{
		// 	toast('Please enter the Date of Anniversary.')
		// }
		//var from_type =$('input[name="profilepage_from_radio_self"]:checked').val();
		var from_type = user.point_claim;

		var mandate=false;

		
		
	if(typeof from_type==undefined || from_type=='undefined'|| from_type==null|| from_type=='null' || from_type=='')
	{
		toast('Please check claim type.');
	}
...
function renderTemplate(pageID, json) {
	...
	case 'loyaltynew':
		...
		document.getElementById('redeemable_points').innerHTML = json.balance;
		document.getElementById('balancepoint').innerHTML = parseInt(checkforundefinedPts(json.balance))+parseInt(checkforundefinedPts(json.burnpoints));
...
function receivedEvent(id) {
 ...
 var push = PushNotification.init({ "android": {"senderID": "796450677270"}});
...
{
	var ref = window.open('https://www.youtube.com/@ACCCementOfficial', '_blank');
}
...
function logoutApp()
{
	...
	document.getElementById("update_photo_self").src = 'assets/images/user_pic.png';
...
//    });
	...
	function showpass() {
		...
		function cancel_Pfrom()
		{
			$("#filter_panel2").slideUp("fast");
			$(".overlay").fadeOut("fast");
		}

		function cancel_Pfrom2()
		{
			$("#filter_panel22").slideUp("fast");
			$(".overlay").fadeOut("fast");
		}
	  ...
	  $.ajax({
	   ...
	   //called when complete
		...
		// console.log(JSON.stringify(data));
			...
			{
                // document.getElementById('bpoint').innerHTML=user.balance;

      
				str +='<a href="javascript:toggleDiv5(\'mypassbook_user_1\', \'mypassbook_user_1_btn\')" class="primary_block_4 text-decoration_none bg-grey d-block" id="mypassbook_user_1_btn">';
				str +='<div class="points_bal_btn p_0 right">';	
				str +='<div class="points_bal">';
				str +='<h4><strong>Transaction History</strong></h4>';
				str +='</div>';						
				str +='<div class="points_arrow"><img src="assets/images/arrow_green.png" class="img-responsive" alt=""></div>';
				str +='</div>';
				str +='</a><br>';

				str += '<div class="out_wrapper2" id="mypassbook_user_1" style="display:none;">';
				 ...
				 {	
						...
						str += '<p class="recent_detail_text">Points Earned from ATOOT BANDHAN</p>';

						//str += '<p class="recent_date">TX Date : '+data.data[i].TransactionDate+'</p>';
						str += '<p class="recent_date">Transaction  ID : '+data.data[i].BillNumber+'</p>';
						str += '<p class="recent_date">Purchased From : '+data.data[i].StoreName+'</p>';
						str += '<p class="recent_date">Brand : '+data.data[i].BrandCode+'</p>';
						str += '<p class="recent_date">Purchase Quantity : '+checkforundefined(data.data[i].Quantity)+'</p>';
                        str += '<p class="recent_date">Purchase Status : '+checkforundefined(data.data[i].TransType)+'</p>';
						str += '<p class="recent_date">Date : '+checkforundefined(data.data[i].BillDate)+'</p>';
						// str += '<p class="recent_date">IHB Name : '+checkforundefined(data.data[i].IHB_Name)+'</p>';
						str += '</div>';
			...
			if(data.bonuspoint[0].msg=='No Records Found.')
			{
				toast('No Records Found.');
			}
			else
			{
				...
				str2 +='<a href="javascript:toggleDiv5(\'mypassbook_site_1\', \'mypassbook_site_1_btn\')" class="primary_block_4 text-decoration_none bg-grey d-block" id="mypassbook_site_1_btn">';
				str2 +='<div class="points_bal_btn p_0 right">';	
				str2 +='<div class="points_bal">';
				str2 +='<h4><str2ong>Bonus History</str2ong></h4>';
				str2 +='</div>';						
				str2 +='<div class="points_arrow"><img src="assets/images/arrow_green.png" class="img-responsive" alt=""></div>';
				str2 +='</div>';
				str2 +='</a>';

				str2 += '<div class="out_wrapper2" id="mypassbook_site_1" style="display:none;">';
                
				 for (var i = 0; i < data.bonuspoint.length; i++)
				 {	
					
				
					    str2 += '<div class="recent_transfer">';
					    // str2 += '<div class="recent_transfer_header pl-4 pr-4">'+data.data[i].BillDate+'</div>';
					    str2 += '<div class="recent_block pl-4 pr-4">';
            
						str2 += '<div class="recent_user_icon"> <img src="assets/images/pointredeem.png" class="img-responsive" alt=""/> </div>';
						str2 += '<div class="recent_user_details">';						
						str2 += '<p class="recent_detail_text">Points Earned from ATOOT BANDHAN</p>';

						//str2 += '<p class="recent_date">TX Date : '+data.data[i].TransactionDate+'</p>';
						str2 += '<p class="recent_date">Transaction  ID : '+data.bonuspoint[i].MobileNumber+'</p>';
						// str2 += '<p class="recent_date">Purchased From : '+data.bonuspoint[i].Flag+'</p>';
						// str2 += '<p class="recent_date">Brand : '+data.bonuspoint[i].BrandCode+'</p>';
						// str2 += '<p class="recent_date">Purchase Quantity : '+checkforundefined(data.data[i].Quantity)+'</p>';
                        // str2 += '<p class="recent_date">Purchase Status : '+checkforundefined(data.data[i].TransType)+'</p>';
						str2 += '<p class="recent_date">Date : '+checkforundefined(data.bonuspoint[i].CreatedDate)+'</p>';
						// str2 += '<p class="recent_date">IHB Name : '+checkforundefined(data.data[i].IHB_Name)+'</p>';
						str2 += '</div>';
						str2 += '<div class="recent_user_points"><h2 class="text_green">'+data.bonuspoint[i].Bonus_Points+'<span>Points</span></h2></div>';						
					    str2 += '</div>';
				        str2 += '</div>';
				 }
				 ...
				 document.getElementById('siteApprovePoint').innerHTML=str2;
...
//  var email = $('#enquiry_email').val();
 ...
 var enuiry_type = $('input[name="enuiry_type"]:checked').val();
   console.log(enuiry_type);
 var enttype = $('#enquiry_type').val();
 var address = $('#enquiry_address').val();
 var pincode = $('#enquiry_pincode').val();
 var enqdt = $('#enquiry_dt').val();
 var enttime = $('#enquiry_time').val();


 if(uname.trim()=='' || mobile.trim() == '' || enuiry_type == '' || enuiry_type == undefined)
 {
     toast('Please enter mandatory fields!');
 }
 else
...
$(document).on('pageshow', '#service_enquiry', function (event, ui) {
    
	$('input[type=radio][name=drone]').change(function() {
		var serviceAvail=$('input[name="drone"]:checked').val();
...
{
	var selectIHB =$('#'+str).val();
	var dataIHB = selectIHB.split(' ');
	console.log(dataIHB);
	// var ihbno = $('#'+str).val();
	console.log(dataIHB[0]);
   if(dataIHB[0]!='none')
   {
	var d1 = parseInt(dataIHB[1]);
	var d2 = parseInt(checkforundefinedPts(dataIHB[2]));
	// console.log(typeof dataIHB[1] + '  ' +typeof dataIHB[2])
	  if(d1 > d2 || d2 == '')
	  {
		$('#tx_ihbmobile').val(dataIHB[0]);
		if(dataIHB[0].length == 10){
            // toast('correct mobile number');
			$.ajax({
				url: SERVER2+'get_ihb_customer_details.asp?apiuid=MLOYALAPI&apipswd=Ml0yalAP!2o14&mobileno='+dataIHB[0],
				type: 'GET',
				timeout: 50000,
				dataType: 'json',
				data: { },
			   success: function(data, textStatus, xhr) {	
					//    console.log('JSONDATA:'+JSON.stringify(data));
					   ...
					   $('#tx_ihbmobile').val(data.sms[0].IHBMobileNo);
		...
		toast('Transaction can not submit on this site.');
	 }
    }
	else if(dataIHB[0] == 'none')
	{
 ...
 $.ajax({
	...
	success: function(data, textStatus, xhr) {
		...
		if(data.length>0)
		{
			...
			document.getElementById("messagepg1").innerHTML = listItems;
		...
		{     
			  ...
			  document.getElementById("messagepg1").innerHTML = 'No msg Available!!';
		}
 ...
 $.ajax({
	...
	success: function(data, textStatus, xhr) {
		...
		if(data.length>0)
		{
			...
			document.getElementById("messagepg2").innerHTML = listItems;
		...
		{     
			  //localStorage.setItem("notseen_count","0");
			  document.getElementById("messagepg2").innerHTML = 'No msg Available!!';
		}
...
function showpointsdetail(){
	...
	if(str == 'none')
	{
		$("#showpointdetail").css("display","none");
	}
	else
	{
		$("#showpointdetail").css("display","block");
	}
}
function scanner(){
	cordova.plugins.barcodeScanner.scan(
...
{
  console.log(user.csostateId+ ' ' +'SHOW DISTRICT LIST');
  $.ajax({
    url: GSERVER2+'Get_alldistrict_json_api.asp',
    type: 'GET',
    timeout: 50000,
    dataType: 'json',
    data: {'stateid': user.csostateId},
    complete: function(xhr, textStatus) {
    },
    success: function(data, textStatus, xhr) {
     console.log(data);
  ...
  $.ajax({
    ...
    success: function(data, textStatus, xhr) {
		...
		for (var i = 0; i < data.data.length; i++)
		{
			...
			if(data.data[i].Districtid == user.district)
				listItems+= "<option value='" + data.data[i].Districtid + "' selected>" + data.data[i].Districtname + "</option>";	
			else
				listItems+= "<option value='" + data.data[i].Districtid + "'>" + data.data[i].Districtname + "</option>";
...
function fetchCityList(district123)
{
	// alert(user.city);
  $.ajax({
    url:SERVER2+'Get_allcity_json_api.asp?districtid='+district123,
    type: 'GET',
    timeout: 50000,
    dataType: 'json',
...
{
	console.log('xdtguiol')
  var stateid = user.csostateId;
  console.log(stateid);
  $.ajax({
...
function showDistrictList_new()
{
  ...
  $.ajax({
    ...
    success: function(data, textStatus, xhr) {
		...
		for (var i = 0; i < data.length; i++)
		{
				listItems+= "<option value='" + data[i].District_Id + "'>" + data[i].District_Name + "</option>";
		}
				  $("#tx_district").html(listItems);
      },
      error: function(xhr, textStatus, errorThrown) {
...
function showCityDealerList(){
	console.log("show dealer");
	var city = $("#tx_city").val();
    console.log(city)

  $.ajax({
    url:SERVER+'store_locator_json.asp?city='+city,
    type: 'GET',
    timeout: 50000,
    dataType: 'json',
...
function showditrictDealerList()
{
	
  $.ajax({
    url:SERVER+'store_locator_json.asp?district='+dist_id,
    type: 'GET',
    timeout: 50000,
    dataType: 'json',
...
function showIhbMemberList(){
	console.log('get data');
	$.ajax({
		url:SERVER2+'get_ihbdetail_by_contractor.asp?apiuid=MLOYALAPI&apipswd=Ml0yalAP!2o14&mobileno='+user.mobile,
		type: 'GET',
		timeout: 50000,
		dataType: 'json',
	   ...
	   var data1 = data.sms;
			...
			{
				// console.log(data[i].storename);
					listItems+= "<option value='" +data1[i].IHBMobileNo+' '+data1[i].Total_Cement_Potiantial+' '+data1[i].Total_Consumption +"'>" + data1[i].IHBName + ' ' +'('+data1[i].IHBMobileNo+')' + "</option>";
			}

			listItems+="<option value='none'> Registration a New IHB </option>";
...
function showproDistrictCityList()
{
  ...
  var pdistrictid = $('#update_district').val();
  $.ajax({
...
function existingUser_check_points(){
	var searchcust = $('#username_new').val();
	console.log('check wallet points'+ ' ' + searchcust)
	  ...
	  $.ajax({
		...
		success: function(data, textStatus, xhr) {
			...
			// fetchcurrent_Location();
             if(data.data[0].BalancePoints > '0'){
				document.getElementById('balancepoint').innerHTML = data.data[0].BalancePoints;
			 }
			// if(data.data.length>0)
			// {
...
{
				var b_Image1 = document.getElementById('update_photo');
				...
				var b_Image3 = document.getElementById('update_photo_self');
				b_Image3.style.display = 'block';
				b_Image3.src = imageURI;

				var b_Image4 = document.getElementById('update_photo_contact');
				b_Image4.style.display = 'block';
				b_Image4.src = imageURI;
...
function site_Reg()
{
 ...
 var matrialstatus = $('#sreg_matrialstatus').val();
//  var mob = parseInt(mobile);
//   console.log(mob.lentgh);
 var numbers = /^[0-9]+$/;
      if(mobile.match(numbers) || mobile == '' || mobile.length !='10'){
     
	  if(sname=='' ){
        toast('Please enter mandatory fields!');
      }else if(matrialstatus=='' || matrialstatus==null || matrialstatus=='undefined' || matrialstatus=='null'){
	    toast('Please enter mandatory fields!');
      }else{
				document.getElementById("create_new_site").disabled = true;
			$.ajax({
				url: SERVER2+'Site_Registration_API.asp',
				type: 'GET',
				timeout: 50000,
				...
				success: function(data, textStatus, xhr) {
							...
							$('#sreg_name').val('');
							$('#sreg_mobile').val('');
							document.getElementById("create_new_site").disabled = false;
				},
				error: function(xhr, textStatus, errorThrown) {
					toast('Could not connect to Server, make sure you are connected to Internet'+textStatus+': '+errorThrown);
				}
...
function openProfession_ptype2(){

	$(".overlay").fadeIn("fast");
	$("#filter_panel22").slideDown("fast");

}

$(document).on("pageshow", "#profilepage2", function(){

	$("input[name='profilepage_from_radio2']").on("change", function(){
		console.log($(this).val());
		$("#select_ptype2").val($(this).val());
	});
...
$(document).on("pageshow", "#profilepage", function(){
	...
	$("input[name='profilepage_from_radio']").on("change", function(){
		console.log($(this).val());
		$("#select_ptype").val($(this).val());
	});
...
function uploadPhoto_kycdoc(imageURI) 
{
				...
				ft.upload(imageURI, encodeURI("https://taghash.co/ACC/upload_photo_new.php"), win, failPhoto_kycdoc, options, true);
}
...
function addTxns()
{
	...
	{
		toast('You cannot post transactions');
	}
}
...
function showPoP(msg){
    ...
    $("<div />", {
        ...
        "<div class=\"ui-block-solo\" style=\"padding: 0 10px\"> <a href=\"javascript:logoutApp()\" class=\"btn btn_primary\" id=\"popnot\">OK</a> </div>"+
      "</div>"
    }).appendTo($popUp);
...
$(document).on('pageshow', '#SiteRegistration', function (event, ui) {
    
    //loadState_list_sregg();
	//fetchDistrictlist_sreg_siterg();

});

function loadState_list_sregg()
{
	    console.log('statelist');
	$.getJSON(
            GSERVER22+'Get_allstate_json_api.asp',
		    ...
		    function (json) {
				 ...
				 var listItems= "";
				     ...
				     for (var i = 0; i < json.data.length; i++)
					 {
						 if(user.state!='')
						 {
							 $('#sreg_state').prop('disabled', true);

							 if(json.data[i].stateid == user.state)
								listItems+= "<option value='" + json.data[i].stateid + "' selected>" + json.data[i].statename + "</option>";	
							 else
							   listItems+= "<option value='" + json.data[i].stateid + "'>" + json.data[i].statename + "</option>";
						 }
						 else
						 {
							   $('#sreg_state').prop('disabled', false);
...
function fetchDistrictlist_sreg_siterg()
{
...
function showDistrictCityList_siteregg_fetch(districtid)
{
...
function checkapp_Version_popup()
{
	...
	str +='</div>';
	str +='</div>';	
	$("#page-forgot-password").prepend(str);
	
}

function closePop_ver()
{
	$(".notification_section").remove();
}
